...
export default function App() {
  ...
  const backendText =
    backendOk === null ? "检测中..." : backendOk ? "在线（8080）" : "离线（8080）";

  const backendBadgeStatus =
    backendOk === null ? "processing" : backendOk ? "success" : "error";
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", padding: 16 }}>
      ...
      <Card title="创建自习室（POST /api/rooms）">
        <Form layout="vertical" onFinish={onFinish}>
          ...
          >
            <Input.TextArea
              rows={3}
              placeholder="例如：每天2个番茄钟，互相答疑，完成任务得积分"
            />
          </Form.Item>
      ...
      <Card
        title={`房间列表（${rooms.length}）`}
        extra={<span style={{ color: "#888" }}>GET /api/rooms</span>}
      >
        <List
          loading={loadingList}
          dataSource={rooms}
